...
class PlanningService {
	...
	constructor(stateRepository: StateRepository, dateService: DateService, randomService: RandomService, slackRepository: SlackRepository, configRepository: ConfigRepository){
	...
	async getNextEvent(channelId: string): Promise<Event> {
		...
		const event: Event = {
			...
			declined: [],
			accepted: [],
			invites: await this.createInvites(inviteCandidates)
		}
	 ...
	 */
	async acceptInvitation(userId: string, channelName: string | undefined = undefined): Promise<boolean> {
		const events = await this.getAllEvents();
	 ...
	 */
	async declineInvitation(userId: string, channelName: string | undefined = undefined): Promise<boolean>{
		const events = await this.getAllEvents();
		const event = await this.findEventToRespondTo(events, userId, channelName);
		if (!event) return false;
	...
	private async findEventToRespondTo(events: Event[], userId: string, channelName: string | undefined): Promise<Event | undefined>{
	...
	private async createInvites(users: string[]): Promise<Invite[]>{
